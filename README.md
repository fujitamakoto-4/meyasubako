...
自由民主党 衆議院議員 藤田まこと事務所（埼玉14区：草加市・八潮市・三郷市）が運営する、地域の困りごとの「見える化」サイトです。LINEや直接の相談で届いたお悩みを、事務所で内容を確認のうえ個人情報を伏せて原文に近い形で公開し、件数と解決事例で議員活動を示します。
...
```
LINE（相談を送る）
  → 事務所（受付台帳へ記録・内容の確認・個人情報を伏せる）
  → このリポジトリ（公開データを管理）
  → GitHub Pages（このサイト）
```

相談の受付はLINEおよび直接承っております。事務所が内容を確認し、個人が特定される情報を伏せたうえで公開しております（誹謗中傷・特定個人/団体への攻撃・クレーム的なもの・公序良俗違反・営業目的のものは非公開）。公開データの管理はGitHub（このリポジトリ）を活用し、`docs/data/*.json` を更新してサイトに「みんなのお悩み」「数字で見る」「解決事例」として表示しています。更新は事務所の受付台帳（Google スプレッドシート）から Google Apps Script が日次で書き込みます。詳しい公開の考え方は [POLICY.md](./POLICY.md) を参照してください。
...
- `total` は取消・対象外（非公開基準に該当するもの）を除く受付件数（公開を希望しなかった件数も含む）。
...
公開しないものの基準・マスキング方針・件数のみを公開する理由などは [POLICY.md](./POLICY.md) にまとめています。
